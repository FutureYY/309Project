...
def model_train_GBC(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, gradient_boosting_params: dict) -> pd.Series:
    classifier_gbc = GridSearchCV(GradientBoostingClassifier(**gradient_boosting_params))
...
def report_evaluation(y_pred_rfc: pd.Series, y_pred_blr: pd.Series, 
    ...
    ROC_AUC_gbc = roc_auc_score(y_test, gradient_boosting_model.predict_proba(X_test)[:, 1])
